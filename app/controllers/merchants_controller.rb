class MerchantsController < ApplicationController
  ...
  def create
    ...
    merchant = Merchant.find_by(email: auth_hash["info"]["email"])  # Considering we only have Github OAuth single login method
    if merchant         # Merchant was found in the database
      flash[:success] = "Successfully logged in as existing user #{merchant.username}"
    else                # Merchant doesn't match anything in the DB
